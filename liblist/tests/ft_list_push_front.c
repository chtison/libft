...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/04/01 16:47:32 by chtison           #+#    #+#             */
/*   Updated: 2016/04/01 17:19:03 by chtison          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_test1(t_list *list)
{
	...
	ret = (size_t)list->pop_front(list);
	ft_assert(ret, 1, "list->pop_front(list)");
	ft_assert(list->count, 2, "list->count)");
	ret = (size_t)list->pop_front(list);
	ft_assert(ret, 2, "list->pop_front(list)");
	ft_assert(list->count, 1, "list->count");
	ret = (size_t)list->pop_front(list);
	ft_assert(ret, 3, "list->pop_front(list)");
...
int			main(void)
{
	...
	printf("\033[34m### \033[1mft_list_push_front(:, :)"
		" && ft_list_pop_front(:)\033[0m\n");
	ft_test1(list);
